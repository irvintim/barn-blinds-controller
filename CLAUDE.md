...
**`docs/project-context/vsp-wsc4-todo.md`, section "Rev 1.4 PCB Changes"** (opened 2026-09-03). Five items with evidence: the `ISO_SHn_*`/`SHn_*` net rename (pure rename, zero copper), **making the output stage survive an UNKNOWN load** (governing requirement set 2026-09-03 — the shade motors are characterised and are *not* the design case; a non-shade motor destroyed board 1, and a general-purpose product cannot know what gets plugged in), proving the stall-detection path (**the fuse was never meant to be the stall protection — the current reading is, which is why the ACS723/ACS725 exists; settled early, do not re-flag it**), hold-up for U1 so motor inrush cannot brown out the ESP32, test points on `+12V`/`+3.3V_MOTOR` **plus net-name silkscreen on every TP**. Add new Rev 1.4 items there, not to the bring-up status log. **J3 is closed (stays DNP), via-in-pad is closed (resolved long ago), and the enclosure work lives on the Enclosure Rev 3 list — do not re-add any of them.**
...
Pruned 2026-08-27: `feature/rework-vias` (fully merged, 0 unique commits vs `main`; its work is in `release/v1.2`'s history at `45e6a61`/`6322e81`). Note it enlarged vias and rerouted. **Via-in-pad was separately resolved long ago by moving vias to pad edges — CLOSED, do not re-flag it.** Measured 2026-09-03: of 177 vias, 104 have their *centre* over a pad, but 48 of those straddle the pad edge with the body hanging out, 33 more sit on large pads (≥1 mm², where it is moot), and the remaining 23 are on 0402 pads (0.9 × 0.95 mm) with only 0.10–0.20 mm of clearance inside the edge — i.e. pushed as far as the geometry allows. The old "96 of 177 vias sit inside SMD pads" figure counted via centres and conflated "centred in the pad" with "shoved to the rim"; it overstated the issue and should not be repeated.
...
4. **Laminate Tg — settled: Rev 1.3 shipped on S1000H TG155.** That is the fact of record; nothing further is open. (A longer ranking of JLCPCB stackup options was written into this file on 2026-08-07 as model-generated analysis, not from a decision — it rested partly on a via-in-pad claim that has since been retracted, and it was cut on 2026-09-03. At ~70 °C operating every option is far below its Tg, and Tg140 FR-4 ships in millions of hot-environment consumer products.)
