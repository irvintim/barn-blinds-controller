...
**Fabrication outputs: re-run and fully verified 2026-08-07 12:40.** Upload the `VSP-WSC-4_1.3_*` set from `production/` — gerbers (11 layers + PTH/NPTH drills, outline 65.004 × 73.935 mm, PTH min drill 0.300 mm), `VSP-WSC-4_1.3_bom.csv` (40 lines / 106 placements) and `VSP-WSC-4_1.3_positions.csv` (106 rows, matches the BOM exactly). Full checklist in `production/JLCPCB-ORDER-NOTES.md`. The older `bom.xls` / `bom.xlsx` / `bom.csv` in that folder are superseded — do not upload them. `gerbers/` and `gerbers-v1.1/` are stale v1.1-era folders.

**BOM duplicate-designator gotcha (corrected 2026-08-07):** a BOM with 16 duplicate designators turned up during the order review. It did **not** come from the Fabrication Toolkit — the toolkit's `VSP-WSC-4_1.3_bom.csv` has never had duplicates. It came from `bom.xls`, whose header is `Comment | Designator | Footprint | JLCPCB Part #`, i.e. the **JLCPCB Assembly Order** format returned by JLCPCB's own parts-matching flow. So: uploading the toolkit CSV directly is safe; if you ever download and re-upload JLCPCB's assembly-order BOM, re-check it for duplicates first.
