...
**`docs/project-context/vsp-wsc4-todo.md`, section "Rev 1.4 PCB Changes"** (opened 2026-09-03). Five items with evidence: the `ISO_SHn_*`/`SHn_*` net rename (pure rename, zero copper), motor-driver protection (D6-D9 SMAJ15CA clamp ~24 V vs the TB6612FNG's 15 V absolute-max VM), fuse and stall-threshold sizing, hold-up for U1 so motor inrush cannot brown out the ESP32, test points on `+12V`/`+3.3V_MOTOR` **plus net-name silkscreen on every TP**. Add new Rev 1.4 items there, not to the bring-up status log. **J3 is closed (stays DNP), via-in-pad is closed (resolved long ago), and the enclosure work lives on the Enclosure Rev 3 list — do not re-add any of them.**
